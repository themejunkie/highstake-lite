 ...
 * @since      1.0.0
 */
class Highstake_Views_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-highstake-views posts-thumbnail-widget',
			'description' => esc_html__( 'Display the most views posts with thumbnails.', 'highstake' )
		);

		// Create the widget.
		parent::__construct(
			'highstake-views',                                        // $this->id_base
			esc_html__( '&raquo; Most Views Posts Thumbnails', 'highstake' ), // $this->name
					...
					echo '<li>';
						...
						echo '<div class="entry-info">';
							...
							if ( $instance['show_view'] && function_exists( 'entry_views_get' ) ) :
								echo '<span class="entry-view">- ' .  entry_views_get( array( 'after' => esc_html__( ' Views', 'highstake' ) ) ) . '</span>';
							endif;
						echo '</div>';
					echo '</li>';
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Most Views Posts', 'highstake' ),
			'limit'     => 5,
			'show_date' => false,
			'show_view' => false
		);
	...
	?>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id( 'show_view' ); ?>">
				<?php esc_html_e( 'Display post view?', 'highstake' ); ?>
			</label>
		</p>
